	...
	function displayTweets($type) {
		...
		if (mysqli_num_rows($result) == 0) {
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				echo "<div class='tweet'> <p><a href='?page=publicprofiles&userid=".$user['id']."'>".$user['username']." <span class='time'>".time_since(time() - strtotime($row['datetime']))." ago</span>: </a></p>"; 
				echo "<p>".$row['tweet']."</p>";
				if ($type != 'yourtweets' && $row['userid'] != $_SESSION['id']) {
					...
					echo "<div class='deleteTweetDiv'><a href='#' class='deleteTweet' id='deleteTweet' data-tweetId='".$row['id']."'>Delete Tweet</div>"; 
				}
				echo "</a></p> </div>";  
			}
		}
